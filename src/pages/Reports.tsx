...
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, PieChart, Pie, Cell, LineChart, Line, DonutChart } from 'recharts';
import { TrendingUp, TrendingDown, DollarSign, Download, Calendar, BarChart3, PieChart as PieChartIcon, AlertTriangle, Users, ShoppingCart, Package, Clock } from 'lucide-react';
  ...
  const fetchAllData = async () => {
    try {
      ...
      const [
        ...
        getDocs(collection(db, 'alterations'))
      ]);
      ...
      // Process alterations
      const alterations = alterationsSnapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      }));
      ...
      // High expense alert
      const avgMonthlyExpense = totalExpenses / Math.max(1, Math.ceil((endDate.getTime() - startDate.getTime()) / (30 * 24 * 60 * 60 * 1000)));
      if (avgMonthlyExpense > 10000) {
        generatedAlerts.push({
          id: 'high-expenses',
          type: 'expense',
          title: 'High Expenses',
          description: `Monthly expenses are above ₹10,000`,
          severity: 'medium',
      ...
      if (absentStaff > staff.length * 0.3) {
        generatedAlerts.push({
          id: 'staff-attendance',
          type: 'staff',
          title: 'Low Attendance',
          description: `${absentStaff} staff members are absent today`,
          severity: 'high',
          count: absentStaff
        });
      }
